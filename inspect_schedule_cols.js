...
async function check() {
    const { data, error } = await supabase.from('ef_schedule').select('*').limit(1);
    if (error) {
        console.error(error);
    } else {
        console.log('Colunas de ef_schedule:', data[0] ? Object.keys(data[0]) : 'Sem registros');
        console.log('Registro exemplo:', data[0]);
    }
}
