...
async function run() {
    ...
    // 1. Atualizar ELETIVA ORIENTADA (MÓDULO INFO IIIA) para SALOMÃO
    console.log('\n1. Atualizando Eletiva Orientada IIIA para o Professor Salomão...');
    const { data: updateSalomao, error: err1 } = await supabase
        .from('ef_classes')
        .update({ teacher_id: 'salomao' })
        .eq('id', '1a498f48-27db-42d6-8334-ff2519f7ca9d')
        .select();
        
    if (err1) console.error('Erro na atualização para Salomão:', err1.message);
    else console.log('Sucesso:', updateSalomao);

    // 2. Definir teacher_id = null para as duplicatas e placeholders de Denilson
    const idsToUnlink = [
    ...
    console.log('\n2. Removendo atribuições extras do Prof. Denilson (definindo teacher_id = null)...');
    const { data: updateUnlinked, error: err2 } = await supabase
        .from('ef_classes')
        .update({ teacher_id: null })
        .in('id', idsToUnlink)
        .select();
        
    if (err2) console.error('Erro ao remover atribuições extras:', err2.message);
    else console.log(`Sucesso: ${updateUnlinked.length} disciplinas desvinculadas.`);

    console.log('\n=== VERIFICAÇÃO FINAL DAS ATRIBUIÇÕES DE DENILSON NO BANCO ===');
    const { data: finalClasses } = await supabase
        .from('ef_classes')
        .select('*')
        .eq('teacher_id', '015a3da3-4abe-409b-ba98-25386247fc39');
        
    console.log(`\nDenilson possui agora ${finalClasses.length} disciplinas vinculadas no banco:`);
